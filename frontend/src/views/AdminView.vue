...
import { createAdminProduct, createAdminVariant, deleteAdminVariant, getAdminOrders, getAdminOverview, getAdminProducts, updateAdminOrderStatus, updateAdminProduct, updateAdminVariant, type AdminOrder, type AdminProduct } from '../services/adminService'
...
const newVariant = ref({ label: '', volume_ml: 50, price: 0, stock: 0 })
...
const openVariantForm = (productId: number, price: number) => { variantProductId.value = productId; newVariant.value = { label: '', volume_ml: 50, price, stock: 0 } }
const createVariant = async (product: AdminProduct) => {
  try { product.variants = [...(product.variants ?? []), (await createAdminVariant(product.id, newVariant.value)).data]; variantProductId.value = null } catch (error) { errorMessage.value = error instanceof Error ? error.message : 'No se pudo crear la variante.' }
}
const changeVariantStock = async (_product: AdminProduct, variant: NonNullable<AdminProduct['variants']>[number], stock: number) => {
  try { variant.stock = (await updateAdminVariant(variant.id, { stock })).data.stock } catch { errorMessage.value = 'No se pudo actualizar la variante.' }
}
const removeVariant = async (product: AdminProduct, variantId: number) => {
  try { await deleteAdminVariant(variantId); product.variants = (product.variants ?? []).filter((variant) => variant.id !== variantId) } catch { errorMessage.value = 'No se pudo eliminar la variante.' }
}
onMounted(load)
</script>
