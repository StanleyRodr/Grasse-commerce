...
import { clearAuthToken, getAuthUser, logout, updateProfile } from '../services/authService'
...
const saveProfile = async () => {
  ...
  profileError.value = ''
  try {
    const updated = await updateProfile(profileForm.value.name, profileForm.value.email)
    if (user) Object.assign(user, updated)
    profileMessage.value = 'Perfil actualizado correctamente.'
  } catch (error) { profileError.value = error instanceof Error ? error.message : 'No pudimos actualizar tu perfil.' }
